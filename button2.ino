...
#define lancPin 4
#define recButton 2
#define irisplusButton 12
#define irisminusButton 9
#define focusButton 13
#define LEDPin 5
...
void setup() {
  ...
  pinMode(irisplusButton, INPUT);
  pinMode(irisminusButton, INPUT);
  pinMode(focusButton, INPUT);
  digitalWrite(recButton, HIGH); //turn on an internal pull up resistor
  digitalWrite(irisplusButton, HIGH); //turn on an internal pull up resistor
  digitalWrite(irisminusButton, HIGH); //turn on an internal pull up resistor
  digitalWrite(focusButton, HIGH); //turn on an internal pull up resistor
...
void loop() {
  ...
  if (!digitalRead(irisminusButton)) {
    IRISMINUS(); //send IRISMINUS command to camera
    delay(0); //debounce button
  }
  if (!digitalRead(irisplusButton)) {
    IRISPLUS(); //send IRISPLUS command to camera
    delay(0); //debounce button
  }
  if (!digitalRead(focusButton)) {
    FOCUS();
    delay(0);
  }
}
...
void IRISMINUS() {
  ...
  while (cmdRepeatCount < 5) {  //repeat 5 times to make sure the camera accepts the command
    ...
    //Write the 8 bits of Byte 1
    //"33hex" or “00110111” sends the  iris increment command
    //Note that the command bits have to be put out in reverse order with the least significant, right-most bit (bit 0) first
    digitalWrite(cmdPin, HIGH);  //Write bit 0
    delayMicroseconds(bitDuration);
    digitalWrite(cmdPin, LOW);  //Write bit 1
